...
<template>
  <section v-if="data" class="w-full px-4 sm:px-6 lg:px-8 py-8">
    ...
    >
      <img
        src="/landing-page-3/hero-mask.png"
        alt=""
        class="absolute left-1/2 top-[65%] w-[90%] sm:w-3/4 md:w-2/3 lg:w-1/2 -translate-x-1/2 -translate-y-1/2 object-contain pointer-events-none"
      />

      <div
        class="absolute inset-0 bg-linear-to-b from-navy-gradient/0 via-navy-gradient/45 to-navy-gradient"
      ></div>
      ...
      <div class="relative z-10 w-full">
        ...
        >
          <h1
            class="text-cream text-3xl lg:text-4xl lg:text-[50px] xl:text-[60px] font-semibold leading-snug whitespace-pre-line"
          >
            {{ data.hero.title }}
          </h1>
          ...
          <Button
            class="rounded-full bg-cream text-navy p-6 hover:bg-cream/90 transition-colors duration-300"
          >
            {{ data.hero.buttonText }}
          </Button>
        </div>
