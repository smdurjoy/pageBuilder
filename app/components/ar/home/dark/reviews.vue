    ...
    <div class="mt-10">
      ...
      >
        <CarouselContent class="px-4 md:px-6">
          <CarouselItem
            v-for="(rev, idx) in data?.reviews.items"
            :key="rev.review || idx"
            class="basis-full sm:basis-1/2 md:basis-1/3 pl-6 md:pl-8"
          >
            <Card
              class="py-8 sm:py-12 md:py-16 px-6 sm:px-8 flex flex-col gap-6"
            ...
            >
              ...
              <p
                class="text-cream w-full font-semibold text-base lg:text-lg"
              >
                "{{ rev.review }}"
              </p>

              <div class="space-y-2">
                <h3 class="text-gold text-lg font-bold">{{ rev.name }}</h3>
                <p class="text-cream font-semibold">{{ rev.role }}</p>
              </div>
            </Card>
          </CarouselItem>
        </CarouselContent>
