  ...
  <section v-if="data" class="w-full px-4 sm:px-6 lg:px-8 py-8">
    ...
    >
        ...
        >
          ...
          <div class="hidden items-center gap-8 text-base font-medium xl:flex">
            <a
              ...
              :key="item.name"
              :href="item.link"
              :class="`transition-opacity hover:opacity-80 duration-300 text-[${heroTextColor}]`"
            >
              {{ item.name }}
            </a>
          </div>
      ...
      <div class="relative z-10 w-full">
        ...
        >
          <h1
            :class="`text-[${heroTitleColor || heroTextColor}] text-3xl lg:text-4xl lg:text-[50px] xl:text-[60px] font-semibold leading-snug whitespace-pre-line`"
          >
            {{ data.hero.title }}
          </h1>

          <p
            :class="`text-[${heroTextColor}] leading-7 md:text-xl lg:text-2xl`"
          >
